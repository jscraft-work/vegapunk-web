...
## 목표
기획서 14.2(C)(D)의 지식 열람·편집·검색·태그 API를 구현한다. 노트 저장 시 동기 인덱싱과 미해결 링크 해결을 03/08과 일관되게 처리한다.
...
```
- 수동 저장은 **동기 인덱싱**(저장 직후 검색 가능). 수정 저장은 현재 본문을 교체한다.
...
- [ ] `tests/test_notes.py::test_update_replaces_current_body` — 수정 저장 시 현재 본문 교체+재인덱싱.
